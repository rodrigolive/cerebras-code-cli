...
**Example:**
- API endpoint lacks proper error handling for edge cases
- Search functionality is missing input validation
...
**Example format:**
- Identified 12 missing test scenarios in `src/services/user.test.ts`
- Current coverage: 60% of code paths
- Missing: empty inputs, special characters, large payloads
- Similar issues exist in `src/services/post.test.ts` (~8 scenarios)
...
**Affected files:**
- `src/services/user.ts:45` - validation method
- `src/api/routes/user.ts:12` - endpoint handler
- `src/services/__tests__/user.test.ts` - existing tests

**Related components:**
- EmailService (depends on user validation)
- AuthMiddleware (authorization checks)
